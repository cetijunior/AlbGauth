...
const Navbar = ({ isSidebarOpen, toggleSidebar, showInput, onQuestionSubmit, }) => {
    ...
    return (
        <nav className="shadow-md w-auto px-4 py-2 border-b border-gray-900 flex flex-col items-center justify-between fixed top-0 left-0 right-0 z-50"
        ...
        >
                ...
                <div className="flex items-center">
                    <Link to="/" onClick={handleHomeClick} className="transition-all duration-500 ease-in-out transform">
                        <img
                            ...
                            className="transition-all duration-500 ease-in-out h-12 w-12 sm:h-12 sm:w-auto"
                        />
                    </Link>
                </div>
            ...
            <div
                ref={menuRef}
                className={`absolute ${showInput ? 'top-[129px]' : 'top-[72px]'} left-0 right-0 bg-gray-900 shadow-lg p-4 md:hidden transition-all duration-500 ease-in-out transform ${menuOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'
                    }`}
            >
